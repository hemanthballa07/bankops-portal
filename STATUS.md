...
- **Frontend spec coverage — fraud-ops screens + service contracts (2026-06-01)** — commits `b7d50bb` (fraud-review + cases), `68a89f3` (dashboard), service specs pending commit
  ...
  - Test-only (no production code touched). Suite **157/157 green** (was 34). Commits: `b7d50bb` (fraud-review+cases), `68a89f3` (dashboard), `97ac12a` (tx+case services), `88fa021` (login), `6e774cd` (customer/reports/agent-admin services), `0258c48` (account service + customers/customer-detail/account-detail screens), + this commit (transaction-filter). Remaining untested = presentational leaf components only (`case-timeline`/`audit-timeline` load+render, `spending-summary`/`monthly-chart` chart compute, trivial `page-header`) + `AuthService` (side-effecty constructor; covered indirectly). Learning captured: `CasesComponent` imports `MatDialogModule`, so its standalone injector resolves the **real** `MatDialog` — a root-level `useValue` spy is shadowed; spy on the component's injected instance (`component['dialog']`) instead, or the real dialog renders `CaseTimelineComponent` and fails on missing `HttpClient`.
