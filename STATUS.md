...
- **Frontend spec coverage — fraud-ops screens + service contracts (2026-06-01)** — commits `b7d50bb` (fraud-review + cases), `68a89f3` (dashboard), service specs pending commit
  ...
  - Test-only (no production code touched). Suite **151/151 green** (was 34). Commits: `b7d50bb` (fraud-review+cases), `68a89f3` (dashboard), `97ac12a` (tx+case services), `88fa021` (login), `6e774cd` (customer/reports/agent-admin services), + this commit (account service + customers/customer-detail/account-detail screens). Learning captured: `CasesComponent` imports `MatDialogModule`, so its standalone injector resolves the **real** `MatDialog` — a root-level `useValue` spy is shadowed; spy on the component's injected instance (`component['dialog']`) instead, or the real dialog renders `CaseTimelineComponent` and fails on missing `HttpClient`.
