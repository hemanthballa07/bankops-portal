...
- **SLA-config admin — Step-4 new screen (2026-06-02)** — branch `feat/sla-config-admin`
  - Makes the previously-hardcoded `SlaPriority` durations (P1=24h/P2=72h/P3=7d) **runtime-editable by ADMIN** without redeploy. DB-backed override (`SlaConfig` entity / `sla_config` table, mirrors the `FeatureFlag` precedent — table via `ddl-auto`, no Flyway in this repo). `SlaConfigService.getDuration()` returns the override **or falls back to the enum default** (behaviour unchanged until edited). `SlaConfigController` `GET`/`PUT /api/admin/sla-config`, `/admin/**`→`hasRole(ADMIN)`.
  - **New-cases-only semantics:** `SlaService.initializeSla` now reads `SlaConfigService.getDuration` (new cases pick up edits), and `computeSlaStatus` was refactored to derive the SLA window from the case's own fields (`(slaDueAt−createdAt)−totalPaused`) instead of the live config — so an edit can't retroactively move an in-flight case's AT_RISK/BREACHED state. The refactor is regression-safe: all 15 pre-existing `SlaServiceTest` cases stay green.
